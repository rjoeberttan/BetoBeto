...
app.post("/placeBet", (req, res) => {
  ...
  db.query(sqlQuery, [gameId, marketId], (err, result1) => {
    if (err) {
      logger.error(
        "Process 1: Error in placeBet request, from accountId:" +
          accountId +
          " error:" +
          err
      );
      ...
      logger.warn(
        "Warn in placeBet request from accountId:" +
          accountId +
      ...
      db.query(sqlQuery, [cummulative, accountId], (err, result2) => {
        if (err) {
          logger.error(
            "Process 2: Error in placeBet request, from accountId:" +
              accountId +
              " error:" +
              err
          );
          ...
          logger.warn(
            "Warn in placeBet request from accountId:" +
              accountId +
              " accountId not found. marketId:" +
              marketId
          );
          res.status(409).json({
            message: "Bet not placed successfully. Please check accountId",
          });
        } else {
          ...
          db.query(
            sqlQuery,
            [description, marketId, gameId, accountId, stake, cummulative, 0],
            (err, result3) => {
              if (err) {
                logger.error(
                  "Process 3: Error in placeBet request, from accountId:" +
                    accountId +
                    " error:" +
                    err
                );
                ...
                logger.warn(
                  "Warn in placeBet request from accountId:" +
                    accountId +
                    " bet not placed marketId:" +
                    marketId
                );
                res.status(409).json({
                  message: "Bet not placed successfully. Please try again",
                });
              } else {
                const betId = result3.insertId;

                logger.info(
                  "Bet placed successfully from accountId:" +
                    accountId +
                    " betId:" +
                    betId
                );
                ...
                db.query(sqlQueryAgent, [accountId], (err, resultAgent) => {
                  if (err) {
                    logger.error(
                      "Process 4: Error in getting commission details of agent of account_id:" +
                        accountId +
                        " err:" +
                        err
                    );
                    ...
                    logger.warn(
                      "Process 4: No associated agent for accountId:" +
                        accountId +
                        " to give commissions to from betId:" +
                        betId
                    );
                    ...
                    logger.info(
                      "Increasing agent wallet, agentId: " +
                    ...
                    db.query(
                      ...
                      (err, resultAgent2) => {
                        if (err) {
                          logger.error(
                            "Process 5: Error in increasing wallet for agent: " +
                              resultAgent[0].account_id +
                              "  to wallet:" +
                              agentCummulative +
                              " for betId:" +
                          ...
                          logger.warn(
                            "Process 5: Warn in increasing wallet, update not successful, for agent: " +
                              resultAgent[0].account_id +
                              "  to wallet:" +
                              agentCummulative +
                              " for betId:" +
                          ...
                          logger.info(
                            "Wallet increased successfully for agentId: " +
                              resultAgent[0].account_id +
                              " to:" +
                              agentCummulative +
                              " for betId:" +
                          ...
                          db.query(
                            ...
                            (err, resultAgent3) => {
                              if (err) {
                                logger.error(
                                  "Process 6: Error in inserting commission transaction for agent: " +
                                    resultAgent[0].account_id +
                                    " for betId:" +
                                    betId +
                                    " err:" +
                                ...
                                logger.warn(
                                  "Process 6: Warn in inserting commission transaction, update not successful, for agent: " +
                                    resultAgent[0].account_id +
                                    " for betId:" +
                                    betId
                                );
                                ...
                                logger.info(
                                  "Insert into commissions table successful, transactionId:" +
                                    resultAgent3.insertId
                                );
                              }
                            }
                          );
                    ...
                    db.query(
                      ...
                      (err, resultMasterAgent) => {
                        if (err) {
                          logger.error(
                            "Process 4: Error in getting commission details of master agent of account_id:" +
                              accountId +
                              " err:" +
                              err
                          );
                          ...
                          logger.warn(
                            "Process 4: No associated master agent for accountId:" +
                              accountId +
                              " to give commissions to from betId:" +
                              betId
                          );
                          ...
                          const masterCommission =
                            parseFloat(
                              parseFloat(stake / 100) *
                                parseFloat(resultMasterAgent[0].commission)
                                ...
                                db.query(
                                  ...
                                  (err, resultMasterAgent3) => {
                                    if (err) {
                                      ...
                                      resultMasterAgent3.affectedRows <= 0
                                    ) {
                                      logger.warn(
                                        "Process 6: Warn in inserting commission transaction, update not successful, for agent: " +
                                          resultMasterAgent[0].account_id +
...
app.post("/settleColorGameBets", (req, res) => {
  ...
  db.query(sqlQuery, [gameId], (err, result) => {
    if (err) {
      logger.error(
        "Process 1: Error in settleColorGameBets request for gameId:" +
          gameId +
          " " +
          err
      );
      ...
      logger.warn(
        "Warn for /settleColorGameBets, game not found gameId:" + gameId
      );
      res.status(409).json({
      ...
      db.query(sqlQuery2, [gameId, marketId], (err, result2) => {
        if (err) {
          logger.error(
            "Process 2: Error in settleColorGameBets request for gameId:" +
              gameId +
              " " +
              err
          );
          ...
          db.query(updateWalletQuery, (err, result3) => {
            if (err) {
              logger.error(
                "Process 3: Error in settleColorGameBets request for gameId:" +
                  gameId +
                  " " +
                  err
              );
              ...
              logger.info(
                "Wallet Balance has been updated for settling marketId:" +
                  marketId +
                  " gameId:" +
                  gameId +
                  " result: " +
...
app.post("/sendTip", (req, res) => {
  ...
  db.query(sqlQuery, [toDecrease, accountId], (err, result1) => {
    if (err) {
      logger.error(
        "Process 1: Error in sendTip request, from accountId:" +
          accountId +
          " error:" +
          err
      );
      ...
      logger.warn(
        "Warn in sendTip request from accountId:" +
          accountId +
          " accountId not found."
      );
      res.status(409).json({
      ...
      db.query(
        ...
        (err, result2) => {
          if (err) {
            logger.error(
              "Process 2: Error in sendTip request, from accountId:" +
                accountId +
                " error:" +
                err
            );
            ...
            logger.warn(
              "Process 2 Warn in sendTip request from accountId:" +
                accountId +
                " accountId not found"
            );
            res.status(409).json({
...
app.get("/getBetHistory/:accountId/:dateFrom/:dateTo", (req, res) => {
  ...
  db.query(sqlQuery, [accountId, dateFrom, dateTo], (err, result) => {
    if (err) {
      logger.error(
        " Process 1: Error in getBetHistory request from accountId:" + accountId
      );
    } else {
